...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for _ in range(self.max_iter_):
            ...
            misclassified = next((i for i in range(n_samples) if y[i] * np.dot(X[i], self.coefs_) <= 0), None)
            if misclassified is None:
                break

            self.coefs_ += y[misclassified] * X[misclassified]
            self.callback_(self, X[misclassified], y[misclassified])
